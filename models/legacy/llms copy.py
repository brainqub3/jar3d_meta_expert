...
class ClaudeModel(BaseModel):
    ...
    def invoke(self, messages: List[Dict[str, str]]) -> str:
        ...
        # print(colored(f"\n\n DEBUG: ClaudeModel payload: {json.dumps(payload)}\n\n", "red"))
        try:
            request_response_json = self._make_request(self.model_endpoint, self.headers, payload)
...
#         # Determine whether to use prompt caching
#         use_prompt_caching = self.prompt_caching

#         # Add beta header only if prompt caching is enabled
...
#             if self.json_response:
#                 payload["messages"][0]["content"] += " Your output must be JSON formatted. Just return the specified JSON format, do not prepend your response with anything."
