...
class Jar3d(BaseAgent[State]):
    def __init__(self, model: str = None, server: str = None, temperature: float = 0, 
        ...
        self.llm = self.get_llm(json_model=False, prompt_caching=True)
...
class ToolExpert(BaseAgent[State]):
    ...
    def __init__(self, model: str = None, server: str = None, temperature: float = 0, 
                 model_endpoint: str = None, stop: str = None, location: str = None, rag_mode: str = "Non-RAG"):
        super().__init__(model, server, temperature, model_endpoint, stop, location, rag_mode)
    ...
    def use_tool(self, mode: str, engine: str, tool_input: str, meta_prompt: str = None, query: list[str] = None, rag_mode: str = "Non-RAG") -> Any:
        if mode == "serper":
            if engine == "search":
            ...
            nodes = None
            relationships = None
            print(colored(f"\n\n DEBUG Retreival Mode: {rag_mode}\n\n", 'green'))

            if self.rag_mode in ["Hybrid", "Dense"]:
                print(colored(f"\n\n DEBUG: We are running the RAG TOOL!!\n\n", 'red'))
                results = run_rag(urls=tool_input, allowed_nodes=nodes, allowed_relationships=relationships, query=query, rag_mode=self.rag_mode)
            elif self.rag_mode == "Non-RAG":
                print(colored(f"\n\n DEBUG: We are running the Basic Scraper TOOL!!\n\n", 'red'))
                results = scraper(url=tool_input)
