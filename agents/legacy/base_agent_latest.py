...
class BaseAgent(ABC, Generic[StateT]):
    def __init__(self, model: str = None, server: str = None, temperature: float = 0, 
                 model_endpoint: str = None, stop: str = None, location: str = "us", rag_mode: str = None):
        self.model = model
        self.server = server
    ...
    def get_llm(self, json_model: bool = False, prompt_caching: bool = False):
        if self.server == 'openai':
            ...
            return ClaudeModel(temperature=self.temperature, model=self.model, json_response=json_model, prompt_caching=prompt_caching)
